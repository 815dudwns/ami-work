...
const DATASET_REGISTRY = [
    ...
    // 고압철거는 2026-09-09 저녁 지도에서 내렸다(영준님 지시). 239계기 중 완료 199·불가 35·
    //   보류 1 로 **미착수가 4건뿐**이라 지도에 띄워 둘 할 일이 남지 않았다.
    //   ★파일은 지우지 않는다 — 누적 실적이라 통계 분모에 그대로 있어야 하고, 같은 리스트가
    //     다시 오면 재사용한다. Firebase 의 `|고압` workStatus 215개도 작업 이력이라 남긴다.
    //   ★그래서 줄을 주석 처리하지 않고 `onMap: false` 로 둔다. 주석 처리하면 stats_sources()
    //     가 이 파일을 못 읽어 통계 분모에서도 함께 빠진다(지시는 '지도에서만 내린다' 였다).
    //     지도에 되살리려면 onMap 을 지우고 map.js 의 ALL 에 '고압' 을 다시 넣으면 된다.
    ...
    //   ★고압과 같은 이유로 주석 처리가 아니라 `onMap: false` 다 — 주석 처리하면 통계 분모에서도 빠진다.
