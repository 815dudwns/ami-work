...
function renderMetersList() {
    ...
    metersList.innerHTML = filtered.map(meter => {
        ...
        // 계약종별 — 데이터에 이미 전건 들어 있었는데 화면에 나오는 곳이 없었다(영준님 2026-09-10
        //   "그대로 디테일에 넣어"). 카테고리로 걸지 않는다 — 값이 실린 리스트면 어디든 나온다.
        //   ★가공하지 않는다. 원천이 달라 형식이 갈리는데 그대로 두는 것이 지시다:
        //     합동(awms) = '100 주택용' 처럼 코드+명칭 · 실효(한전 엑셀) = '주택용전력' 명칭만.
        //     맞춰 다듬으면 어느 쪽이 원본인지 화면에서 알 수 없게 된다.
        //   ※skt·고압·장애는 원본에 이 칸이 없어 전 건 빈값이다 — 아무것도 안 그린다.
