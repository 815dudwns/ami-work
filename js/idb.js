// js/idb.js — 작은 IndexedDB key-value 래퍼 (siteData 캐시-우선 로더용, Part 2)
//   목적: 16MB site-data.json을 폰 IndexedDB에 캐시 → 버전 같으면 재다운로드 0.
//   localStorage는 5~10MB 한도라 16MB 못 담음 → IndexedDB 사용.
//   모든 함수는 실패 시 reject — 호출측이 try/catch로 fetch 폴백해야 함(누락 금지).
(function () {
    const DB_NAME = 'ami-cache';
    const STORE = 'kv';
    const DB_VER = 1;
