...
const NAMESPACED_CATEGORIES = ['고압', '합동', 'skt', '장애', 'LP무기록', '미청구', '미청구불가'];
...
function statusKeyOf(item, index) {
    const addr = item.주소;
    const key = index
        ? (index.byMarker.get(_lookupKey(markerKeyOf(item), addr)) || addr)
        : addr;
    if (!String(key == null ? '' : key).trim()) return fallbackKeyOf(item);
    return key;
}
