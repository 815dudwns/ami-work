...
- **LP무기록(확인용)** = 보강현황에서 한전이 숨긴 행 중 우리 기록 없이 LP 만 올라온 개소 2,054건.
  **`allowGroup:'staff'` — 우영준·윤용운(user09) 에게만 보인다.** 계정을 더 열 때는
  `js/auth.js` 의 `AUTH_GROUPS` **한 줄만** 고친다(판정은 `authAllowsGroup()` 한 곳이 한다).
  확인이 끝나면 레지스트리 줄과 데이터 파일을 함께 지운다.
