...
- **배포 상태**: tongsin `13e5590` — **커밋만 하고 푸시는 안 했다.** PM 이 main 에 머지·
  통계인덱스 생성·푸시한다. 9/20 작업은 전부 커밋 완료, 게이트 전부 통과.
- **데이터 현황 (2026-09-20 마감 기준, 다음 반영 때 대조값)**

  | 데이터셋 | 지도 | 대기 |
  |---|---:|---:|
  | 실효 `site-data.json` | **3,166** | — |
  | 합동 `hapdong-data.json` | **943** (9/18 193 · 9/17 353 · 9/16 397) | — |
  | 합동 `hapdong-data-archive.json` | **5,430** | — |
  | **25미청구** `michunggu-data.json` | **8,208** | 195 |
  | **25년 미청구불가** `michunggu-bulga-data.json` | **5,694** | 3 |
  | SKT | 41 | — |
  | (지도 밖) 고압 239 · 장애 323 | — | — |

  ★`archive` 는 **줄면 안 된다**(누적 append). 합동 `h` 는 늘기만 한다.
  ★미청구 2종은 **지도 + 대기 합이 대상 수**다(8,403 / 5,697). 빌더 게이트가 매번 검사한다.
- **합동 보존 경계 = 영업일 3일**, 기준일은 오늘이 아니라 **데이터의 최신 작업일**이다.
  마지막 수집 `data/inbox_hapdong/last_fetch.json` = **20260920** (범위 9/17~9/20).
...
### ★반드시 지킬 것 (2026-09-09~20 에 사고로 배운 것들)
...
- **★25미청구 · 25년 미청구불가 — 리스트 2종 (2026-09-20 신설, 커밋만·미푸시)**
  ...
  - 화면 규칙(영준님 2026-09-20 연속 지시로 굳은 것):
    · **상단 변대주 = DCU 판별 자리. 원천은 `dcu_master` 하나뿐이다.** 변대주명·전산화번호로
      ...
      다섯이 **동수**여야 정상(미청구 5,844 · 불가 3,277). 하나만 채우고 나머지가 비면 버그다.
    · 대장 미등재는 **두 갈래**다 — ①리스트에 DCUID 가 있는데 대장에 없다 → **아무것도 안 쓴다**
      ...
      **'DCU 없음'**(판별 결과다) ③변대주 자체가 없다 → 줄을 안 그린다.
      건수 미청구 125 / 1,604 / 635 · 불가 0 / 1,757 / 660.
