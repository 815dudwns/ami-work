...
## 현재 상태 (2026-09-07)
- **마지막 작업**: 통계 작업자별·구별 집계에서 **완료 아카이브 제외**(`b4f30bd`) — 한 주소에 쌓인
  과거 완료분이 오늘 실적으로 세어져 이용택 16개소가 계기 628 로 잡히던 것(공릉동 714 아카이브 290 ·
  708 아카이브 282). 수정 후 43. · 남의 계기 완료를 물려받은 재 49건 해제(`dbedd4a`) ·
  **데스크 4곳 main 동기화 완료**(`c596791`, 상태판 파일명 분리).
- **진행 중**: 주덕기 과장 **새 실효 리스트 대기**(요청 완료). 도착하면 아래 `다음 0-A` 의 원인별 분해와
  `0-A-0` 의 계기 단위 재 판정 기준으로 재구성한다. 그때 `0-A-1`(재 표시 거르기)도 함께.
- **배포 상태**: ami-work `b4f30bd` / 아미큐 **2.2.17**(교체 M1020) / 종로맵 20260813.3 / 헬퍼 1.0.77·inject v80.
- **지도 데이터**: 실효 3,969 · 재방문 127 · 고압 239 · 합동 849(9/2~9/4, 3일 보존) · 장애 323그룹 2,837계기 · SKT 41.
- **데스크 fan-out 생략(2026-09-07)**: 4곳 모두 8/19 이후 커밋·작업 0이라 깨워서 받을 상태가 없다.
  상태는 git 으로 실측해 아래 '데스크 상태' 에 적었다(뒤처짐 전부 0).

### ★이번 구간에 새로 만든 규칙 (되돌리지 말 것)
- **데이터셋은 `js/datasets.js` 한 곳에서 정의한다** — 지도 마커·카테고리 패널·통계 리스트·통계 인덱스가 전부 이 레지스트리를 읽는다. 파이썬 거울 `scripts/datasets.py`, 어긋나면 `scripts/test_datasets_parity.py` 가 잡는다. 예전엔 같은 정보를 네 군데에 적었고 한 곳을 빠뜨리면 그 리스트만 통계에서 0으로 잡혔다(장애가 실제로 그랬다).
- **통계 개소 = 지번(건물) 단위** — 합동 주소는 awms 가 동호수까지 붙여 줘서 주소로 세면 한 건물 52계기가 개소 50개가 된다(계기 2,893 vs 개소 2,817). 개소 상태는 **전 세대 완료일 때만 완료**로 센다.
- **합동시공은 그날 작업분만, 전지사·공사번호 필터 없음** — 거르는 것은 고압뿐이고 빌더가 계약종별로 한다. 과거 누락분은 소급하지 않는다. 용산·중구 포함(옛 `101`·`155` 제외 규칙 폐기). `scripts/hapdong/fetch_awms.py` DEPTS 가 단일 출처 — 목록 밖에서 임시로 돌린 지사는 다음 수집에서 반드시 빠진다.
- **같은 지번은 좌표를 하나로 모은다**(합동 빌더) — 한전이 같은 건물을 도로명 양식과 지번 양식 두 가지로 올려 좌표가 0.7m 갈리고 마커가 포개졌다. 주소 문자열은 건드리지 않는다(workStatus 키라 바꾸면 완료기록이 끊긴다).
- **장애 그룹 주소는 번지 있는 쪽을 먼저 본다** — 표수만 세면 '중랑구 망우동'(번지 없음)이 이겨 마커가 동 중심에 470m 어긋나 박혔다. 번지 없는 주소는 덜 적힌 것이지 다른 주소가 아니다.
- **아미큐 설치구분 3상태** — 신설 M1010 / 기설 M1030 / **교체 M1020**. 교체는 기존 모뎀맥 `EXT_FCTY_ID`(★`EXT_MAC_MODEM` 아님)와 구분상세 `REMV_MEMO`(M102010 신호미약·M102020 모뎀불량·M102030 마스터 변경)를 함께 싣고, **슬레이브도 전건 M1020 + 같은 기존맥·구분상세**로 나간다.
- **awms DCU_ID 는 조회값이 아니라 화면이 계산하는 값** — `DATA_NUM + (BIZ_DGR 없으면 '6') + 통신방식코드(KS-PLC '4' / WI-SUN '5' / LTE '6' / Zigbee '7' / 그 외 없음)`. 돌려주는 API 가 없다(화면이 부르는 16개 중 없음). 헬퍼는 awms 화면을 띄우니 자동으로 채워지고 아미큐는 화면이 없어 우리가 대신 계산한다. **`'64'`·`'6'` 리터럴로 굳히지 마라** — BIZ_DGR 이 빈값일 때만 나오는 결과다.
- **한전 DCU 대장 ID 와 awms DCU_ID 는 다른 체계다** — 대장 = 변대주번호 + **차수코드**(1차 00·01 / 3차 39·30·31 / 4차 44·41·45 / 5차 5M·5S / 6차 6S·6M, 19,007행 전건 변대주번호로 시작). awms = 변대주 + BIZ_DGR + 통신방식코드. 같은 변대주 `9827P663` 이 대장 `…44` / awms `…64`. **섞어 쓰지 마라.**
- **LTE 개소의 DCU 정보는 무시한다** — 한전이 LTE 전환 후에도 옛 PLC DCU 소속·상태를 계속 실어 보낸다. 순수숫자 10자리 DCUID = LTE 회선번호(대장에 없음)이고, 그런 개소의 `DCU장애여부`("검침실패 3시간 이상" 등)는 판단 근거가 못 된다. 장애 리스트에도 LTE 그룹 28개에 옛 PLC DCU 가 붙어 있다(미정리).
- **LP 빈칸 = 미판정이 아니라 한전 기준으로는 미개통** — 주덕기 과장이 빈칸을 장애 리스트로 넘긴다(장애 2,682건 = 미수신 1,854 + 실패 828, 성공은 0건). 성공률을 낼 때 빈칸을 성공으로 치지 마라.
- **진행상태(조치완료/확정)와 상태(성공/실패)는 별개 축이다** — 조치완료 안에도 실패가 1,614건 있다. 흐름은 조치완료 → LP 측정 → 확정.
- **여러 파일을 순서대로 로드하는 JS 는 합쳐서 실행해 봐야 한다** — `node --check` 는 파일 하나씩만 봐서 `const` 중복선언(전역 공유)을 못 잡는다. 실제로 그것 때문에 지도가 통째로 죽었다(2026-09-04).

### 다음 (우선순위)
0-A. **★주덕기 과장 새 실효 리스트 대기 (요청 완료)** — 받으면 아래 대조 결과를 반영해 재구성한다. 8/28 리스트(7,437건) 대조에서 **어디에도 없는 2,199건**을 원인별로 갈라 두었다:
  · **재(再) 731** — 완료로 찍었는데 LP 0. 8/28 이전 완료분. **재 표시로 실효 복귀 대상**
  · **402** — 8/28 리스트 생성(10:47) 이후 완료. 시차라 정상, 새 리스트에서 자동으로 빠질 것
  · **204** — "awms 등록 있으면 장애로" 규칙으로 실효에서 뺐는데 장애 리스트를 장애 시트로만 만들어 **갈 곳이 없어짐**(규칙 사이 구멍)
  · **549** — 8/12 우선순위 정리로 내려간 것(교체일 전부 6월 이전). 한전이 8/28 에 다시 올려보냈다 → 우선순위 해제 여부 확인 필요
  · **91** — 단순 누락 / 183 불가 / 39 보류·대기(정상)
  ★재구성 시 함정 3가지: ①**6/8 컷오프는 재방문 분리 기준이지 실효에서 빼는 기준이 아니다**(이번에 549건이 그렇게 빠졌다) ②**어디로 뺄지 정했으면 받는 쪽이 실제로 받는지 확인**한다 ③**리스트 생성시각 이후 완료분은 분모에서 제외**하고 센다
0-A-0. **★재(再) 판정은 계기 단위로 — 주소 previousComplete 는 판정에 쓰지 않는다 (2026-09-07 확정)**
  **판정축 = awms 모뎀작업리스트(계기별 시공기록).** 그 계기가 awms 에 등록돼 있고 보강현황에 다시
  나왔으면 재, 등록이 없으면 그냥 실효(미시공)다. 지도 완료는 작업자 자기신고이고 **workStatus 키가
  주소라 계기를 못 가린다** — 보조로만 쓴다. 8/31 리스트업 커밋에 이렇게 적어놓고 실제 분류는
  주소 기준 previousComplete 로 해서 어긋났다.
  ★실측(2026-09-07): **현재 재방문 138건 중 awms 시공기록이 있는 것은 0건**이다(93건은 남의 완료를
  주소로 물려받았고 45건은 주소기록조차 없다). 반대로 **실효 3,958건 안에 진짜 재가 23건** 섞여 있다
  (79190718488 망우동 303-1 · 58190021547 홍제동 6-6 · 24530202305 정릉동 397-17 등, awms 성공).
  계기 단위로 다시 가르면 **재 23 / 실효 4,073** 이다.
  ※지금 바꾸면 마커 138개가 옮겨간다 — **새 리스트로 재구성할 때 함께** 한다(두 번 일하지 않는다).
0-A-1. **★남의 완료를 물려받은 '재' 표시 거르기 (리스트업 때 같이)** — workStatus 키가 주소라 한 주소에
  계기가 여럿이면 한 계기 완료가 그 주소 전체에 붙는다. **실측 49건**(재방문 11 · 실효 38)이
  **계기교체일 > 완료일** 이라 그 완료 시점에 존재하지도 않던 계기다(예: 79450105657 교체 6/25 > 완료 6/23).
  주소별로 뭉친다 — 충정로3가 1-38 에 4건, 옥천동 110-1 에 4건.
  ★고칠 곳 = `js/detail.js` 재작업 알림 블록(~L328). **교체일이 이전 완료일보다 늦으면 previousComplete 를
  표시하지 않는다.** 데이터는 안 지운다(표시만 거른다). 영준님 2026-09-07 "리스트업 때 다시 하자".
  구조는 실효+재방문 1,597주소 중 **670주소가 2계기 이상**(3,169계기, 최다 중랑구 묵동 142 에 42계기)이라
  같은 일이 계속 생긴다. 근본 해결은 완료를 계기 단위로 기록하는 것(meterChecks 가 이미 계기 단위).
0-B. **아미큐 교체(M1020) 실전 검증** — 2.2.17 배포했으나 실제 awms 왕복 미확인. 통신팀은 임시저장(25)이 없어 검증하려면 완료(28)로 1건 올린 뒤 `deleteRows` 해야 한다(전송 29 전이면 삭제 자유). 첫 교체건 올릴 때 `getDetail` 로 `EXT_FCTY_ID`·`REMV_MEMO`·슬레이브 `WORK_DIV` 확인.
0-C. **LTE 개소에 붙은 옛 PLC DCU 정리 여부** — 장애 28그룹·실효 다수. 현장에서 "이 DCU 문제인가" 오인 위험. 지우거나 표시를 구분할지 미결.
0-D. **아미맵 완료 아카이브가 7/4 이후 안 만들어지고 있다** — 완료되면 site-data 에서 빼는데 담을 아카이브가 `site-data-completed-archive-20260704.json` 하나뿐. 그래서 완료분 추적이 끊긴다(이번 미배정 조사에서 드러남).
0-E. **합동 중복 3건**(같은 계기가 두 작업일에 등장) — LP타운 103호는 한전이 같은 호수를 새 순번으로 다시 올린 것. 상계동 2건은 계기번호 오입력 의심(다원전력설비). 주덕기 확인 목록.
0-F. **서울본부직할 중구 물량 확인** — 공사번호 필터를 걷어내면서 중구가 들어왔다(9/3 32건·9/4 39건). 용산은 영준님 확인받았으나 중구는 미확인.
...
- **아미고(53) 계기는 KS-PLC 에서 약하다** — 전체 KS-PLC 90.2% vs 아미고 86.2%. 같은 계기가 SMGW-C 에선 97~98%. 2453 KS-PLC 83.5% / 4553 84.1%(단 4553 은 동대문중랑 114건 실패 67 이 끌어내린 것, 그 지사 빼면 93.9%).
- **1차 DCU(접미 00·01)는 성공률이 무너진다** — 2453 KS-PLC 기준 8.3%, 4553 기준 0%. 영준님 "1차는 원래 안 된다" 확인.
- **접미 44 자체는 문제 아니다** — 전체 1,386건 94.4%로 평균. 4차·접미44 가 나빠 보인 건 2453 표본이 몰려 있어서다. 안 되는 곳은 **개소 단위**로 뭉친다(성공 0 개소 12곳 77계기 = 1차 제외분의 20%).
- **DCU_ID 누락은 LP 실패 원인이 아니다** — 변대주 있고 DCUID 없는 건의 성공률 89% vs 있는 건 90% vs 둘 다 없는 건 91%.
- **모뎀작업리스트 변대주는 20%만 차 있다** — 계기번호로 우리 원장에서 변대주명을 끌어오면 40%, 맥그룹 전파까지 하면 63%까지 오른다. 매칭은 **변대주명+지사** 가 1순위.

### ★데이터관리자 설계 확정사항 (유지)
- **봉인**: 7자리 고정 · 박스 범위는 설정 입력(현 `0315001~0320000`) · **계정 무관 팀 자원**(전역 커서) · 다음번호 = 우리 DB 마지막+1, awms 조회값은 범위 안이면 반영·밖이면 무시 · 삼상 N,N+1 · 박스 끝 1개 남으면 버림
- **차수**: 하드코딩 금지, 목록 선택. 기본값만 활성차수 자동추종. 화면엔 공사명+차수
- **전송**: 완료(28) 기본 · 검증 자동 대조 · 완료건 읽기 전용 잠금
- **환경**: 봉인 커서 **0315875**(다음 0315876) / 활성 26차(397820263354) / 계정 mdp2603726 / 백엔드 8765 launchd 자동재기동

### 데스크 상태 (2026-09-07 — main 동기화 완료)
> 8/19 부터 19일간 정지해 있던 데스크 4곳을 **모두 main 에 동기화**했다(뒤처짐 0).
> 그 19일간의 작업(main 62커밋)은 전부 PM 세션이 직접 했다.

| 데스크 | 브랜치 | main 대비 | 데스크 상태판 |
|---|---|---|---|
| 통신팀 | `tongsin` | 뒤처짐 0 · 앞섬 22 | `HANDOFF-통신팀.md` |
| 계기팀 | `gyegi` | 뒤처짐 0 · 앞섬 5 | `HANDOFF-계기팀.md` |
| 검증팀 | `geomjeung` | 뒤처짐 0 · 앞섬 6 | `HANDOFF-검증팀.md` |
| ocr-meter | `ocr-meter` | 뒤처짐 0 · 앞섬 3 | `HANDOFF-ocr-meter.md` |

- **★데스크 상태판 파일명을 갈랐다 (2026-09-07).** 예전엔 데스크도 `HANDOFF.md` 를 써서 main 과 경로가
  같았고, **머지하면 PM 상태판이 데스크판으로 덮였다.** 그게 데스크가 19일간 main 을 못 받은 이유다.
  이제 데스크는 `HANDOFF-<팀>.md`, PM 은 `HANDOFF.md` 다. **되돌리지 마라.**
  ※머지에서 git 이 이름변경으로 인식해 두 문서를 섞는 일이 있었다(통신팀·ocr-meter). 분리 복원 완료.
- 데스크가 앞선 커밋은 **머지할 실질 내용이 없다.** 통신팀 코드(델타 동기화 `js/firebase.js` 518줄)는
  이미 main 에 있고(차이 0), 남은 고유분은 죽은 폴더 `ami-queue-www` 뿐이다.
- **머지 전 워크트리에 있던 미커밋 변경은 `data/desk-parked-20260907/` 에 보관**했다(버리지 않았다).
  계기팀 `site-data.json`(8/12 시절 10,504건)·`rework-data.json`, ocr-meter HANDOFF 2건.
...
- workspaceStatus 는 자동으로 안 바뀐다 — 재우거나 세울 때 `orca worktree set --workspace-status` 로 직접.

### 인프라 (2026-08-19)
- **`.git` 24GB → 5.5GB** (`git gc --prune=now`, 18.5GB 회수·손실 0). 원인 = APK 80개 2.5GB(`cst-input/amiqueue.apk` 하나가 48벌) + 정리 안 된 느슨한 개체 7.83GB + 쓰레기 0.96GB. **남은 정리(미착수)**: `jongno-combined/apk/` 옛 snap APK 24개 약 900MB 삭제 / `git filter-repo`로 히스토리에서 APK 제거(2.5GB, 해시 전부 바뀜→워크트리 4개 재생성 필요) / **APK 배포를 git 대신 GitHub Releases로 전환**(안 하면 계속 쌓인다)
- **Claude 자격증명은 `~/.claude/.credentials.json` 단일 파일 사용자 공유** — 한 세션 `/login`이 전 세션을 복구한다. 세션마다 로그인할 필요 없다. 다만 이미 뜬 세션이 옛 토큰을 물고 있으면 그 세션만 `exit` 후 재실행(워크스페이스 삭제 금지)
- Firebase rules에 `syncAt` 색인 추가(권한 변경 없음). 백업 `data/rules-backup/`
...
## 대기 액션 (영준님 결정/확인 필요)
- **★주덕기 과장 새 실효 리스트 (요청 완료, 도착 대기)** — 8/28 리스트 대조 결과(위 `다음 0-A`)를 그걸로 반영한다.
- **★우선순위 배제 549건 — 해제됐는지 확인 필요** — 8/12 에 "최근 것 우선"으로 내렸던 6월 이전 물량인데 한전이 8/28 리스트에 다시 올려보냈다. 해제면 밀린 일감이고, 유효면 계속 보류다.
- **★서울본부직할 중구 물량** — 공사번호 필터를 걷어내면서 들어왔다(9/3 32건·9/4 39건). 용산은 "용산도 올려" 확인받았으나 중구는 미확인.
- **★LTE 개소에 붙은 옛 PLC DCU 표시 여부** — 한전이 LTE 전환 후에도 옛 DCU 소속·상태를 실어 보낸다. 현장에서 "DCU 문제인가" 오인 위험. 지울지·구분표시할지 미결.
...
## 블로커
- **★데스크 4곳이 8/19 에서 멈춰 있고 미푸시 26커밋을 안고 있다** — 재가동하려면 main 머지가 먼저다(위 '데스크 상태'). 워크트리 삭제 금지.
- **★아미큐 교체(M1020) 실전 왕복 미검증** — 2.2.17 배포했으나 awms 에 실제로 `EXT_FCTY_ID`·`REMV_MEMO` 가 붙는지 확인 못 했다. 통신팀은 임시저장(25)이 없어 완료(28)로 1건 올린 뒤 `deleteRows` 해야 한다.
- **★완료 아카이브가 7/4 이후 생성 안 됨** — 완료되면 site-data 에서 빼는데 담을 파일이 `site-data-completed-archive-20260704.json` 하나뿐이라 완료분 추적이 끊긴다.
