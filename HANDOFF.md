...
### 데스크 상태 (2026-09-07 실측 — 포인터만, 본문 복사 금지)
> ★**데스크 4곳 전부 2026-08-19 에서 멈춰 있고 각자 미푸시 커밋을 안고 있다.** 그 뒤 19일간의 작업(main 62커밋)은
> 전부 PM 세션이 직접 했다 — 영준님이 "오르카 모바일에 상주에이전트가 안 보인다"고 하셔서 직접 작업으로 전환했고
> 그 상태가 이어졌다. 데스크에 다시 일을 맡기려면 **먼저 main 을 머지**해야 한다(안 그러면 낡은 지침·죽은 폴더를 읽는다).

| 데스크 | 브랜치 | main 대비 | 마지막 커밋 | 세션 |
|---|---|---|---|---|
| 통신팀 | `tongsin` | **뒤처짐 62 · 앞섬 19** | 2026-08-19 `f97b3ff` | 살아있음(`아미맵 합동시공 데이터셋 검수 및 배포`) |
| 계기팀 | `gyegi` | 뒤처짐 112 · 앞섬 3 | 2026-08-19 `2625771` | 살아있음(untitled) |
| 검증팀 | `geomjeung` | 뒤처짐 137 · 앞섬 4 | 2026-08-19 `11fd758` | 살아있음(untitled) |
| ocr-meter | `ocr-meter` | 뒤처짐 144 · 앞섬 0 | **2026-07-23** `3c2a736` | 살아있음(untitled) |

- **미푸시 26커밋이 데스크 브랜치에만 있다**(통신 19 · 계기 3 · 검증 4). 워크트리를 지우면 사라진다 —
  **워크트리·워크스페이스 삭제 절대 금지**(같은 경로 형제 세션까지 전멸, 2026-07-18·07-19 실증).
- 데스크 재가동 절차: ①`git -C workspaces/<데스크> log --oneline main...HEAD` 로 앞선 커밋 확인 →
  ②필요분 main 에 반영 → ③`git merge main` → ④그 다음에 일을 준다.
- **PM 우편함 `run:run_48e996df65bd`**(`~/.claude/orca-runs.json`). 고아 Run `run_f566b669c5ef` 는 미회수 상태로 남아 있다.
- workspaceStatus 는 자동으로 안 바뀐다 — 데스크를 재우거나 세우면 `orca worktree set --workspace-status` 로 직접 옮긴다.
