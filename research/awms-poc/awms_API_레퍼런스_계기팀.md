...
### ★★ 등록 범위 — 25/28 선택식 (PM 최종지시 §1-7, 2026-07-29)

**2026-07-20의 "임시저장(25) 한정" 제약은 해제됐다. 이제 완료(28)까지 등록한다.**
어디까지 올릴지는 **화면에서 고른다** — `POST /transmit/run`의 `work_step`(`"25"` / `"28"`).

| 값 | 의미 | 되돌리기 |
|---|---|---|
| `"25"` (기본) | 임시저장까지 | `resetRows`로 원복 가능 |
| `"28"` | 완료까지 | **★불가.** 검침값 수정만(`mobMtr5000/saveRow`, `EX_WORK_STEP=28`+`RE_SAVE_YN=Y`), 신설번호 잠김 |

- 완료(28) 경로: 신설 저장 후 `getDetail` 재조회 → 시공 17키 → `RE_SAVE_YN` → 신설 사진 재전송.
- 기본값을 `"25"`로 둔 이유: 되돌릴 수 있는 쪽이 안전 기본값이다. 28은 호출측이 명시해야 나간다.
- **28은 삭제 불가이므로 전송 전 확인을 강화한다**(화면 책임): 완료로 나갈 건수 명시,
  "완료는 되돌릴 수 없습니다" 경고, **검증완료된 건만** 대상, 봉인 범위 초과 차단.
- 구현: `app.py` `TransmitRunReq.work_step` → `no_complete = (work_step != "28")` → `awms_mtr_direct`.
  잡 로그에 어느 단계로 나가는지 표시한다.
- ★`resetRows`는 여전히 **25 전용**. 완료건 원복 자동화는 **설계상 만들지 않는다.**

### CONS_NO (차수 — 종로 사업)

> ★**차수 번호를 코드·설정에 하드코딩하지 않는다** (영준님 확정 2026-07-29).
> 아래 표는 **참고용 스냅샷**일 뿐 진실의 출처가 아니다. 현재 차수는 항상 awms에서 조회한다:
> **`mobMtr8000/getMainList`(봉인조회) 응답의 `LV_CONS_NO`** = 활성 차수.
> 드롭다운 목록은 `mobMtr1000/getBusiList?DEPT1=3970`로 매번 최신 조회(27차·28차가 생기면 자동 등장).

실측 스냅샷 (2026-07-29 getBusiList, 종로 `39782026…`):

| CONS_NO | 차수 | 지역/비고 |
|---|---|---|
| 397820263023~3026 | 1~4차 | |
| 397820263032 | 5차 | 청운동, 신교동 |
| 397820263033 | 6차 | 궁정/효자/창성/통의/적선/통인동 |
| 397820263034 | 7차 | 누상동, 누하동 |
| 397820263035 | 8차 | 옥인/체부/필운동 |
| 397820263103 | 9차 | 중구 |
| 397820263133 | 10차 | 중구 검기만료 |
| 397820263143 | — | 중구 저압 실효계기 교체공사 |
| 397820263150 | 12차 | |
| 397820263151 | 13차 | |
| 397820263153 | 14차 | |
| 397820263219 | 20차 | |
...
| 397820263292 | 23차 | |
| 397820263350 | 24차 | |
| **397820263354** | **26차** | 2026-07-29 시점 활성(`LV_CONS_NO`). ★고정값 아님 — 넘어가면 바뀐다 |
...
#### 등록 차수 결정 규칙 (전송탭 구현)
1. 사용자가 드롭다운에서 활성차수와 **다른** 차수를 고르면 → `cons_no_manual=true`로 잠기고 **그 선택이 최우선**.
2. 그 외에는 호출 시점에 `LV_CONS_NO`를 **실시간 조회**해 따라간다(저장값 의존 없음).
3. 조회 실패 시에만 마지막으로 알던 값으로 폴백.
- 구현: `_resolve_cons_no()` / `mtr_direct.get_active_cons_no()` (`research/admin-validation/backend/`).
- ★사고 이력: 예전엔 login-snapshot이 "cons_no가 비어있을 때만" 채워 **23차가 고착** → 26차에 배정된 계기를 23차로 오등록(2026-07-29 P4). 그 자리에 다른 고정값을 넣지 않고 **출처를 바꿔** 해결했다.
- 계기별 저장: 선택된 차수는 `awms_seal.cons_no`로 계기 레코드에 남는다(§ 데이터 보존 참조).
