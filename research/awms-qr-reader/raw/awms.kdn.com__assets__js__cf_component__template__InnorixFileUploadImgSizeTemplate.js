...
Vue.component("innorix-file-upload-img-size", {
	props: ["id"],
	data: function() {
		return {
            ...
            reader.onload = function(e) {
				...
				img.onload = () =>{
					...
					canvas.toBlob((blob) => {
			            ...
			            if(self.capturingImgWithCameraInifo.imgObject.src){
							URL.revokeObjectURL(self.capturingImgWithCameraInifo.imgObject.src);
						}
			            self.capturingImgWithCameraInifo.imgObject.src = URL.createObjectURL(compressedFile);
			            self.capturingImgWithCameraInifo.isChanged = true;
						self.capturingImgWithCameraInifo.imgObject.parentElement.classList.add('ami-img-wrap');
						self.fileControl.fileList.getAllFiles()[0].file = compressedFile;
						self.fileControl.fileList.getAllFiles()[0].fileSize = compressedFile.size;
					}, 'image/jpeg', 0.8);
				}
            }
