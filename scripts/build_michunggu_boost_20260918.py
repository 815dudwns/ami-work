...
class Sources:
    ...
    def _load_ledger_mac(self, L):
        ...
        for r in L.to_dict('records'):
            ...
            # ★고객번호는 여기서 **가져오지 않는다**(영준님 2026-09-23 "고객번호로만").
            #   주소·변대주·지사는 같은 모뎀이면 물리적으로 같은 자리라 공유해도 되지만,
            #   **고객번호는 계약에 붙는 값**이라 한 함체 안에서도 계기마다 다르다.
            #   실측 2026-09-23: MAC 01248584738 함체에서 Amigo 계기의 고객번호가
            #   3상 계기 두 건으로 옮겨붙었고(타입이 다르니 같은 계약일 수 없다),
            #   그 틀린 고객번호를 키로 상호·공동주택명·인입주까지 2차 오염됐다.
            #   전체 135개 고객번호 / 374계기가 겹쳤다(한 고객번호에 최대 11계기).
            #   고객번호는 우리 **매칭 1순위 키**라([[ledger_match_key_rule]]) 오염되면
            #   그 키로 거는 판정이 전부 없는 매칭을 만든다.
