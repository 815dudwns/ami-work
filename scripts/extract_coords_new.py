...
좌표 캐스케이드는 scripts/geocode_cascade.py 로 옮겼다(2026-08-10). 카카오 도로명 → 지번 →
정규화 지번 → 키워드 → **네이버** → 본번 → 동 중심 순. 여기서 직접 카카오만 때리던 옛 코드는
(1) 네이버 폴백이 없어 카카오에 없는 지번을 전부 동 중심으로 떨어뜨렸고
(2) 동 추출 정규식이 "서울특별시 **동**대문구"의 첫 '동'에 걸려 엉뚱한 구 좌표를 넣었다.
"""
...
def resolve(key):
    ...
    실제 조회는 geocode_cascade.resolve 가 한다. 실패해도 fail 로 돌려주고 행은 버리지 않는다.
    """
