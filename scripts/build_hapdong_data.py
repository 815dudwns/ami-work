...
WORK_DAY_OVERRIDES = [
    ...
    # 광진성동 소급분 — 우리가 수집을 시작하기 전(8월 초)에 한 작업이라 한 번도 안 올라갔다.
    #   영준님 지시 2026-08-26 "저번주꺼 싹 긁어서 25일로 반영".
    #   ★실제 작업일이 열흘 넘게 흩어져 있어 그대로 두면 날짜 트리가 열한 갈래로 갈린다.
    #     그래서 지도용 '작업일' 만 하루로 모으고, **실제 작업일시는 원본 그대로 보존**한다
    #     (상세모달이 '작업일시' 를 먼저 그린다 — js/detail.js).
    #   같은 배치의 8/25 정규 수집분 광진성동은 공사번호 끝이 017·022 라 여기 안 걸린다.
