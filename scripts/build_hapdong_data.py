...
WORK_DAY_OVERRIDES = [
    ...
    # 광진성동 소급분 — 지난주에 한 작업인데 3일 규칙으로 지도에서 빠져 있던 것을 되살린다.
    #   영준님 지시 2026-08-26 "저번주꺼 25일로 반영. 작업날짜 디테일에 쓰고".
    #   ★지도용 '작업일' 만 반영일로 모으고 **실제 작업일시는 원본 그대로 둔다**.
    #     상세모달이 '작업일시' 를 먼저 그리므로(js/detail.js) 현장에서는 실제 날짜가 보인다.
    #     날짜 트리는 지도를 읽는 단위라 하루로 묶어야 갈래가 안 늘어난다.
    #   같은 배치의 정규 수집분 광진성동은 공사번호 끝이 달라 여기 안 걸린다.
    {'batch': '20260825', '지사': '광진성동지사', '공사번호끝': '016', '작업일': '20260825'},
    # 주말 등재분 -> 직전 금요일. 영준님 지시 2026-09-06 "주말에 올라온 건들을 저번주 금요일 일수로".
    #   9/5(토) 서울본부직할 3건(공사번호 끝 181, 중구 회현동1가·예장동)이 실제로는 금요일 작업분이다.
    #   9/6(일) 은 유령 건 하나뿐이라 옮길 것이 없다(좌표·주소가 통째로 빈 CONS_TGT_SEQNO 2494072).
    #   ★날짜 트리를 하루로 모으는 것이고 '작업일시' 원본은 그대로 둔다 — 상세모달에는 실제 날짜가 보인다.
    {'batch': '20260905', '지사': '서울본부직할', '공사번호끝': '181', '작업일': '20260904'},
]
