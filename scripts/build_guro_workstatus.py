...
차수 엑셀(1/3/5/7/9/11/14/15/18차) → workStatus/guro 구조로 변환
...
GURO_DB_DIR = "/Users/woodelight/Projects/ami-work/research/ocr_poc/meterid_goldset/guro_db/"
...
# 차수별 파일 정보 — 차수 오름차순으로 정렬 (dedup: 마지막 차수 = 높은 차수 우선)
# UPLOAD_DIR 파일(UUID명): 5/14/15/18차
# GURO_DB_DIR 파일: 1/3/7/9/11차 (guro_db에 5/14/15/18도 있으나 UPLOAD_DIR 사용)
EXCEL_FILES = [
    ("1차",  os.path.join(GURO_DB_DIR, "계획공사_1차.xlsx")),
    ("3차",  os.path.join(GURO_DB_DIR, "계획공사__3차.xlsx")),
    ("5차",  os.path.join(UPLOAD_DIR,  "4cb72579-__________5__.xlsx")),
    ("7차",  os.path.join(GURO_DB_DIR, "계획공사7차.xlsx")),
    ("9차",  os.path.join(GURO_DB_DIR, "계획공사9차.xlsx")),
    ("11차", os.path.join(GURO_DB_DIR, "계획공사11차.xlsx")),
    ("14차", os.path.join(UPLOAD_DIR,  "fa1283c0-___________14__.xlsx")),
    ("15차", os.path.join(UPLOAD_DIR,  "434215c3-__________15__.xlsx")),
    ("18차", os.path.join(UPLOAD_DIR,  "a8a59e8e-__________18__.xlsx")),
]
...
def parse_excel(cha, fpath):
    ...
    # 헤더 자동 탐지: '순번' 셀이 있는 행 (9차는 row[2], 나머지는 row[1])
    header_idx = None
    for i, row in enumerate(rows[:5]):
        if row and str(row[0]).strip() == '순번':
            header_idx = i
            break
    if header_idx is None:
        raise ValueError(f"{cha} 헤더 행 탐지 실패 (첫 5행에 '순번' 없음)")
    ...
    # 필수 컬럼 확인
    missing = [k for k in ('new_meter', 'old_meter', 'install_date', 'cust_no') if col[k] is None]
    if missing:
        raise ValueError(f"{cha} 필수 컬럼 미탐지: {missing} (헤더={header_idx}행)")
    ...
    for row in rows[header_idx + 1:]:
        ...
        cust_no_val = None
        if col['cust_no'] is not None and row[col['cust_no']]:
            cust_no_val = str(row[col['cust_no']]).strip()
...
def build():
    ...
    cha_counts = {}
    print("\n엑셀 파싱 중...")
    for cha, fpath in EXCEL_FILES:
        recs = parse_excel(cha, fpath)
        if len(recs) == 0:
            raise RuntimeError(f"{cha} 파싱 결과 0건 — 컬럼명 불일치 가능성. 중단.")
        cha_counts[cha] = len(recs)
    ...
    cust_no_filled = sum(
        1 for ws_rec in result.values()
        for mr in ws_rec['replacement_list'].values()
        if mr.get('cust_no')
    )

    print(f"\n========== 빌드 결과 ==========")
    print(f"\n차수별 파싱 건수:")
    for cha, cnt in cha_counts.items():
        print(f"  {cha}: {cnt}건")
    print(f"\n완료 주소: {len(result)}")
    print(f"  complete: {complete_count}, hold: {hold_count}")
    print(f"완료 계기 수: {total_meters}")
    print(f"cust_no 채움: {cust_no_filled}/{total_meters} ({100*cust_no_filled//total_meters if total_meters else 0}%)")
    ...
    for r in all_records:
        c = r['cha']
        cha_matched.setdefault(c, 0)
        cha_unmatched.setdefault(c, 0)
    for addr, ws_rec in result.items():
        for old_id, mr in ws_rec['replacement_list'].items():
            cha_matched[mr['cha']] = cha_matched.get(mr['cha'], 0) + 1
    for u in unmatched:
        cha_unmatched[u['cha']] = cha_unmatched.get(u['cha'], 0) + 1

    print(f"\n차수별 매칭 현황:")
    all_cha = sorted(set(list(cha_matched.keys()) + list(cha_unmatched.keys())))
    for c in all_cha:
        matched = cha_matched.get(c, 0)
        unm = cha_unmatched.get(c, 0)
        total_c = matched + unm
        rate = 100 * matched // total_c if total_c else 0
        print(f"  {c}: 매칭 {matched}/{total_c} ({rate}%)")
    ...
    output = {
        '_meta': {
            ...
            'source': '한전 준공리스트 (1/3/5/7/9/11/14/15/18차)',
