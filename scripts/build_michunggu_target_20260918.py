...
def build_target():
    ...
    hv.discard('')
    # ★고압 행이 있어도 **고압이 아닌 '미청구' 행이 따로 있으면 빼지 않는다**(영준님 2026-09-23).
    #   고압은 옛 이력이고 그 뒤에 일반 보강으로 시공된 개소가 있다 — 계기 단위로
    #   '고압 행이 하나라도 있으면 제외' 하면 그 미청구 행까지 같이 쓸려 나간다.
    #   실측 6,065건 중 4건: 74450062896(25/07 불가 고압 -> 26/06 미청구 신설) ·
    #   29450242649 · 91190148046 · 98190283096
    _nonhv = {nm(m) for m, g, st in zip(L['계기번호'], L['공종'].astype(str), L['상태'].astype(str))
              if g != '고압' and st == '미청구'}
    _rescue = hv & _nonhv
    if _rescue:
        log(f'  고압 — 비고압 미청구 행이 따로 있어 **유지** {len(_rescue)}건 {sorted(_rescue)}')
    hv -= _nonhv
