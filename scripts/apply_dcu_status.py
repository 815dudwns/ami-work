...
def apply_to(records, removal, comm):
    ...
    for x in records:
        # ★변대주만 쓴다. DCU 가 달리는 전주는 변대주이고, 인입주는 계기로 들어가는 다른 전주라
        #   대장의 '변대주명'과 맞출 값이 아니다(영준님 2026-08-12: "인입주는 상관없다").
        #   인입주를 앞에 두는 바람에 변대주로는 걸리는 개소 7건이 철거예정 태그를 못 받았고
        #   (그중 4건이 '해지' 판정 — 철거될 DCU 인데 표시가 없어 PLC 로 시공할 뻔했다),
        #   폴백으로 남겨도 得이 없다 — 변대주가 빈 개소는 인입주도 전부 비어 있다(실측 0건).
        #   남는 건 다른 전주 이름에 잘못 걸릴 위험뿐이라 아예 뺀다.
        name = (x.get('변대주') or '').strip()
